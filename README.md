...
Hago énfasis en lo último, ya que los SDKs necesarios para levantar un servicio de estas características están en constante cambio.  Lo que hace que por ejemplo la función SSEServerTransport, que hace una semana funcionaba, esté deprecada. Probablemente esto generó consecuencias en capas de abstracción más profundas como podría ser una configuración Linux, lo cual derivaría en un error fatal y por eso dan más tiempo, pero la borrarán. Trabajando a nivel Python, C o Java, debiese ser tan simple como llamar a la función actualizada y olvidarse de lo que pasa por debajo. Pero es algo a tener en consideración.
Para testear el funcionamiento de los agentes, levanto el servidor por un puerto http en local, lo expongo a través de Ngrok (al menos OpenAI está bloqueando todas las conexiones desde puertos locales), voy al archivo principal de la carpeta de agentes, pongo el link de ngrok en "tools: [hostedMcpTool({serverLabel: 'turistik-mcp-server', serverUrl: 'LINK DE NGROK'})]," Y ejecuto en agents/ el comando npm run dev. Con eso el sistema es capaz de armar el agente, leer las tools del servidor y operar a un nivel básico. 

## Pasos a seguir

Buscar casos de uso y programar agentes para automatizar procesos. La nueva capa de abstracción que ofrecen las APIs de LLMs, es justamente el lenguaje natural. Por lo cual es clave abstraer este concepto hasta lo más básico, para poder identificar casos de uso adecuados.
